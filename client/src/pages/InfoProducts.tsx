...
// PaypalButtonComponent eliminado de aquí para que solo aparezca en el carrito
...
const InfoProducts = () => {
  ...
  // CORRECCIÓN: Recepción de talla optimizada
  const onAddToCart = () => {
    // Priorizamos la talla del contexto porque SizeButtons actualiza el context global
    const finalSize = contextSize || selectedSize;
    ...
    if (addToCart) {
      ...
      console.log("Musa_Log: Producto añadido con éxito. Talla:", finalSize);
    }
      ...
      <main className="max-w-7xl mx-auto px-6 py-4 pb-32">
        <div className="flex flex-col lg:grid lg:grid-cols-12 gap-12 lg:gap-20 items-start">
          ...
          <div className="w-full lg:col-span-5 flex flex-col space-y-10">
            ...
            {/* SECCIÓN DE TALLAS */}
            <div className="space-y-8 p-8 rounded-[2.5rem] bg-[#111111] border border-white/5 shadow-2xl shadow-black/50">
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Talla Seleccionada</label>
                  <button className="text-[9px] font-bold text-pink-500 uppercase tracking-widest border-b border-pink-500/20 hover:text-white transition-colors">Guía de tallas</button>
                </div>
                    ...
                    <div className="grid grid-cols-5 gap-3">
                      {["XS", "S", "M", "L", "XL"].map((size) => (
                        <button
                          ...
                          className={`h-14 rounded-2xl border transition-all duration-500 font-black text-[11px]
                            ${(contextSize === size) 
                              ? "border-white bg-white text-black shadow-[0_0_25px_rgba(255,255,255,0.15)] scale-95" 
                              : "border-white/10 text-gray-400 hover:border-white/30 bg-white/5 hover:bg-white/10"}`}
                        >
                          {size}
                        </button>
                      ))}
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center w-full sm:w-44 h-14 bg-white/[0.03] border border-white/10 rounded-2xl px-2">
            ...
            <div className="space-y-4">
              ...
              >
                <span className="text-[11px] font-black uppercase tracking-[0.3em]">
                  {isAdding ? "SYSTEM_UPDATED" : "Añadir a la bolsa"}
                </span>
              </button>
              {/* PAYPAL ELIMINADO: Ahora el flujo es más limpio y directo a la bolsa */}
            </div>
